...
const App = () => {
  ...
  return (
    <View style={[styles.container, { width: window.width, height: window.height, backgroundColor: '#1a1a1a' }]}>
        <Text style={[styles.title, { color: '#FFFFFF' }]}>MWD TASKS</Text>
        <FlatList
            data={tasks}
        ...
        <Button title="Список" onPress={() => setShowPredefinedTasks(true)} style={[styles.button, styles.showPredefinedTasksButton]} />
        ...
        <Modal visible={showPredefinedTasks} animationType="slide" transparent={true}>
            <View style={styles.modalContainer}>
                ...
                {predefinedTasks.map((task, index) => (
                    <Text key={task} onPress={() => addPredefinedTask(task)} style={[styles.button, styles.predefinedTaskButton]}>{task}</Text>
                ))}
                </View>
...
const styles = StyleSheet.create({
  ...
  predefinedTaskButton: {
      marginBottom: 4, // Увеличиваем расстояние между задачами на 15
